class BookDetailInfoBeanEntity {
	bool isForbidForFreeApp;
	String copyright;
	String superscript;
	BookDetailInfoBeanRating rating;
	bool allowMonthly;
	int contentLevel;
	dynamic discount;
	String lastChapter;
	...
	String majorCateV2;
	int safelevel;
	List<BookDetailInfoBeanStarrating> starRatings;
	String cat;
	...
	BookDetailInfoBeanEntity.fromJson(Map<String, dynamic> json) {
		...
		if (json['starRatings'] != null) {
			starRatings = new List<BookDetailInfoBeanStarrating>();(json['starRatings'] as List).forEach((v) { starRatings.add(new BookDetailInfoBeanStarrating.fromJson(v)); });
		}
		cat = json['cat'];
...
class BookDetailInfoBeanRating {
	double score;
	int count;
	bool isEffect;
	String tip;

	BookDetailInfoBeanRating({this.score, this.count, this.isEffect, this.tip});

	BookDetailInfoBeanRating.fromJson(Map<String, dynamic> json) {
		score = json['score'];
		count = json['count'];
...
class BookDetailInfoBeanStarrating {
	int star;
	int count;

	BookDetailInfoBeanStarrating({this.star, this.count});

	BookDetailInfoBeanStarrating.fromJson(Map<String, dynamic> json) {
		star = json['star'];
		count = json['count'];
	}
